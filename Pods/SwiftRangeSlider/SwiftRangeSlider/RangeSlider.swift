...
@IBDesignable open class RangeSlider: UIControl {
    ...
    @IBInspectable open var trackTintColor: UIColor = mediumGreyColor {
        didSet {
    ...
    @IBInspectable open var knobSize: CGFloat = 6.0 {
        didSet {
    ...
    @IBInspectable open var trueKnobSize: Bool = true {
        didSet {
            updateLayerFramesAndPositions()
            lowerKnob.setNeedsDisplay()
            upperKnob.setNeedsDisplay()
    ...
    @IBInspectable open var knobBorderThickness: CGFloat = 0 {
        didSet {
            lowerKnob.setNeedsDisplay()
            upperKnob.setNeedsDisplay()
        }
    ...
    @IBInspectable open var curvaceousness: CGFloat = 2.0 {
        didSet {
            track.setNeedsDisplay()
            lowerKnob.setNeedsDisplay()
            upperKnob.setNeedsDisplay()
    ...
    @IBInspectable open var labelFontSize: CGFloat = 12.5 {
        didSet {
            updateLabelText()
    ...
    open func updateLabelPositions() {
        ...
        var newLowerLabelCenter = CGPoint(x: lowerKnobCenter.x, y: lowerKnob.frame.origin.y - (lowerLabel.frame.size.height / 2) - 4.0)
        var newUpperLabelCenter = CGPoint(x: upperKnobCenter.x, y: upperKnob.frame.origin.y - (upperLabel.frame.size.height / 2) - 4.0)
...
/*import UIKit
 ...
 ///The font size of the labels. `12.0` by default.
 @IBInspectable open var labelFontSize: CGFloat = 13.5 {
 didSet {
 updateLabelText()
 updateLabelPositions()
 }
 ...
 */
 ...
 var newLowerLabelCenter = CGPoint(x: lowerKnobCenter.x, y: lowerKnob.frame.origin.y - (lowerLabel.frame.size.height / 2) - 4)
 var newUpperLabelCenter = CGPoint(x: upperKnobCenter.x, y: upperKnob.frame.origin.y - (upperLabel.frame.size.height / 2) - 4)
